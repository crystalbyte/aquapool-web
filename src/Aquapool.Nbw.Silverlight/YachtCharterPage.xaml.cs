...
namespace Nbw
{
	public partial class YachtCharterPage : UserControl
	{
		public YachtCharterPage()
		{
			...
			InitializeComponent();
            this.ContentGrid.Children.Add(new YachtCharterTextPage());
		}

        private YachtCharterGalleryPage EnsureGalleryPanelExistence()
        {
            if (!(this.ContentGrid.Children[0] is YachtCharterGalleryPage))
            {
                this.ContentGrid.Opacity = 0;
                this.ContentGrid.Children.Clear();
                this.ContentGrid.Children.Add(new YachtCharterGalleryPage());
                var panel = this.ContentGrid.Children[0] as YachtCharterGalleryPage;
                panel.Close += new EventHandler(panel_Close);
                var sb = this.Resources["FadeInNbwContent"] as Storyboard;
                sb.Begin();
            }

            return this.ContentGrid.Children[0] as YachtCharterGalleryPage;
        }
